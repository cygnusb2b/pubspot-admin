  ...
  if (env.BRANCH_NAME == 'master') {
    ...
    try {
      stage('Build Container') {
        nodeBuilder = docker.build("pubspot-admin:v${env.BUILD_NUMBER}", ".")
      }
    } catch (e) {
    ...
    try {
      ...
      stage('Deploy Upgrade') {
        rancher confirm: true, credentialId: 'rancher', endpoint: 'https://rancher.as3.io/v2-beta', environmentId: '1a18', image: "664537616798.dkr.ecr.us-east-1.amazonaws.com/pubspot-admin:v${env.BUILD_NUMBER}", service: 'pubspot/admin', environments: '', ports: '', timeout: 300
      }
      stage('Deploy Notify') {
