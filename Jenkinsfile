    ...
    try {
      docker.withRegistry('https://664537616798.dkr.ecr.us-east-1.amazonaws.com', 'ecr:us-east-1:aws-jenkins-login') {
        stage('Build Container') {
          myDocker = docker.build("pubspot-admin:v${env.BUILD_NUMBER}", '.')
        }
      ...
      stage('Upgrade Container') {
        rancher confirm: true, credentialId: 'rancher', endpoint: 'https://rancher.as3.io/v2-beta', environmentId: '1a18', image: "664537616798.dkr.ecr.us-east-1.amazonaws.com/pubspot-admin:v${env.BUILD_NUMBER}", service: 'pubspot/admin', environments: '', ports: '', timeout: 30
      }
